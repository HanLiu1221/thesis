...
The related work to this thesis is in three folders. We will first discuss part-based modeling, as the mainline of our work, closely related to the replaceable substructure analysis which will be introduced in Chapter~\ref{chapter4}; constraint-aware shape analysis is relevant to the interior layout exploration in both Chapter~\ref{chapter3} and Chapter~\ref{chapter4}, will be discussed in section~\ref{related_work:2}. The related work of sketch based design in Chapter~\ref{chapter5} is given in section~\ref{related_work:3}.

%In the recent years, significant progress has been made towards component-based shape analysis and synthesis, particularly man-made objects.
...
\mypara{Automated floor plan generation}
...
\mypara{Man-made object synthesis}
...
%
For the former kind, Chaudhuri et al.~\cite{Chaudhuri2010} enable component retrieval for ill-defined shapes based on geometry matching. Kalogerakis et al.~\cite{Chaudhuri2011,Kalogerakis2012} improve the parts identification by using a probabilistic model incorporating geometric style and semantics to select relevant components and to automate plausible synthesis. Bokeloh et al.\cite{Bokeloh2011} preserve detected regular patterns during deformation using sliding docking sites, leading to the expansion of the shape space. The ShapeSynth system~\cite{averkiou2014shapesynth} extracts a template-based parameterizable space and presents an exploration tool allowing users preview synthesized shapes based on the embedding.
%
For the latter kind,
Kim et al. \cite{Kim12} propose to compute similarities between shapes using fuzzy correspondence, while Huang et al. \cite{Huang12} match shapes by computing point-to-point maps between a pair of models. But it is difficult to directly apply either of the analytic methods to shape synthesis. Kalogerakis et al. \cite{Kalogerakis12} present a probabilistic model abstracting model structure based on which create new shapes with structure variabilities. More recently, Zheng et al. \cite{zheng2014recurring} detect recurring arrangements of parts using semantic consistency.

\mypara{User-driven reshaping}
...
%
Li et al.~\cite{Li_siga12}, in an interesting work, deform input man-made objects to make them amendable to stacking.
